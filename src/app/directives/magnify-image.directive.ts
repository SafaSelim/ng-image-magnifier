...
export class MagnifyImageDirective {
  ...
  magnify() {
    ...
    this.imgElement.parentElement.childNodes.forEach((node:any) => {
    ...
    this.imgElement.addEventListener('mousemove', (e: any) => this.moveMagnifier(e));
    this.lensElement.addEventListener('touchmove',(e) => this.moveMagnifier(e));
    this.imgElement.addEventListener('touchmove',(e: any) => this.moveMagnifier(e));
    ...
    const imgResizeObserver = new ResizeObserver(entries => {
      for (const entry of entries) {
        ...
        this.lensElement.style.backgroundSize =
          cr.width * this.zoom + 'px ' + cr.height * this.zoom + 'px';
      }
  ...
  magnifierActive(event: any, isActive: boolean) {
    if (isActive) {
